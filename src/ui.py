...
def display_sidebar():
    """Cria e exibe a barra lateral para os inputs subjetivos do utilizador."""
    with st.sidebar:
        st.header("Feedback Diário (Pré-Treino)")
        st.write("Informe os dados da sua noite de sono anterior ao treino e como se sente.")
        
        sono_duracao = st.time_input("Duração do Sono", value=time(7, 30))
        sono_profundo = st.time_input("Duração do Sono Profundo", value=time(1, 15))
        sono_score = st.number_input("Pontuação do Sono (0-100)", min_value=0, max_value=100, value=85)
        
        st.divider()
        stress = st.slider("Nível de Stress Geral (1=Baixo, 5=Alto)", 1, 5, 2)
        energia = st.slider("Nível de Energia pré-treino (1=Baixo, 5=Alto)", 1, 5, 4)
...
def display_detailed_analysis(db, user_id, activity_id, access_token, start_date, selected_activity_name, sono_duracao, sono_profundo, sono_score, selected_activity):
...
def get_metrics_to_save(df_com_pace, selected_activity_name, start_date, has_fc, analise_texto, selected_activity):
    ...
    return {
        ...
        "fc_media": float(fc_media), "fc_max": int(fc_max) if has_fc and pd.notna(fc_max) else 0,
        "cadencia_media": float(cad_media), "ascensao_total_m": float(ascensao_total), "analise_ia": analise_texto
    }
...
def display_progress_dashboard_tab(db, user_id):
    ...
    if history_df.empty:
        st.info("Ainda não há treinos guardados na sua base de dados.")
    else:
